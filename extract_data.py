...
data_dict = {d["file_name"]: d for d in data if d["file_name"] in SELECT_COLOR and d['color'] == SELECT_COLOR.get(d["file_name"],DEFAULT_COLOR)}
...
for file_name, position in data_dict.items():
    ...
    def extract(imp,name):
        ...
        save_path = os.path.join(dir_path, file_name.replace(".czi", "-" + name + ".png"))
